...
const addJobModalSchema = Joi.object().keys({
	...
	jobLink: Joi.string()
		...
		.messages({
			...
			"string.uri":
				"Please input valid(full url with https:// or http://) URL",
		}),
...
export default function JobAddModal({ onClose, reload }: JobAddModalProps) {
  ...
  useEffect(() => {
    const fetchCompanyOptions = async () => {
      const companies = await companyService.searchCompanyByName(searchCompanyByName);


      console.log(companies)

      const _remapCompanyOptions = companies.length ? companies.map((company) => ({
		...
		>
          ...
          onInputChange={(event, newInputValue) => {
            ...
            setSearchCompanyByName(newInputValue);
					...
					renderInput={(params) => (
						<TextField required {...params} label="Related Company" variant='standard'  />
					)}
				/>
							...
							<Controller
								...
								render={({ field }) => (
									<TextField
										...
										helperText={
											errors.jobExperienceLevel?.message
										}
										{...field}
									/>
								)}
